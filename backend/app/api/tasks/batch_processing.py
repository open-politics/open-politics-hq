...
Used after directory import to process CSV/PDF extraction without flooding the Celery queue.
"""
...
def batch_process_pending(
    ...
    """
    Process PENDING assets in a bundle in self-chaining batches.

    Processes parent assets only (no parent_asset_id). On success, schedules
    the next batch if more PENDING assets remain.
    ...
    with Session(engine) as session:
        ...
        for asset in assets:
            try:
                run_async_in_celery(processing_service.process_content, asset, {})
                processed += 1
            except Exception as e:
                logger.error(f"[Batch Processing] Failed asset {asset.id}: {e}")
        ...
        if remaining > 0:
            ...
            logger.info(f"[Batch Processing] Completed bundle {bundle_id}")
...
def batch_enrich(
    ...
    ALLOWED_FACETS = {FACET_LANGUAGE, FACET_LOCATION_LAT, FACET_LOCATION_LON, FACET_SUMMARY, FACET_OCR_USED}
