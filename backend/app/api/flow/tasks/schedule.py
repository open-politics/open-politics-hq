...
def _is_task_due(task: Task, now: datetime) -> bool:
    """
    Determine if a task is due to run based on its cron schedule and last_run_at.
    Returns True if the task has never run, or if the next scheduled time has passed.
    """
    if not task.schedule or not task.schedule.strip():
        return False
    if task.last_run_at is None:
        return True
    try:
        base_time = task.last_run_at
        if base_time.tzinfo is None:
            base_time = base_time.replace(tzinfo=timezone.utc)
        cron = croniter(task.schedule.strip(), base_time)
        next_run = cron.get_next(datetime)
        if next_run.tzinfo is None:
            next_run = next_run.replace(tzinfo=timezone.utc)
        return next_run <= now
    except (ValueError, KeyError) as e:
        logger.warning(f"Invalid cron expression for task {task.id}: {task.schedule!r}: {e}")
        return False
...
def check_recurring_tasks():
    ...
    Parses each task's cron schedule and dispatches only when due.
    """
        ...
        try:
            ...
            for task in active_tasks:
                if not _is_task_due(task, now):
                    logger.debug(f"Task {task.id} not due (schedule={task.schedule}, last_run={task.last_run_at})")
                    continue
                try:
...
def _dispatch_embed_task(task: Task):
    ...
    from app.api.search.tasks.embed import embed_infospace_task
