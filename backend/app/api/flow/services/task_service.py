...
def _add_or_update_schedule(
    recurring_task_id: int, schedule_str: str, is_enabled: bool, task_type: Optional[str] = None
) -> None:
    """No-op: Beat schedule registration not implemented. Tasks use check_recurring_tasks beat."""
    pass
...
class TaskService:
    ...
    def create_task(
        ...
        try:
            ...
            # Add the task to the scheduler (no-op; check_recurring_tasks reads from DB)
            _add_or_update_schedule(
    ...
    def delete_task(
        ...
        try:
            if task_id_to_remove is not None:
                ...
                _remove_schedule(recurring_task_id=task_id_to_remove)
            else:
