...
Phase 1–3 pipeline orchestration: metadata extraction, content processing, enrichment.
...
Flow: PENDING → Phase 1 (metadata + type refinement) → Phase 2 (processor) → Phase 3
      (enrichers) → READY. Phase 3 runs language_detection and quality_score enrichers.
"""
...
class ProcessingService:
    ...
    async def _run_phase1_metadata(self, asset: Asset) -> Optional[Dict[str, Any]]:
        ...
        if asset.kind == AssetKind.PDF and asset.blob_path:
            from app.api.processors.pdf_processor import extract_pdf_metadata
    ...
    async def process_content(self, asset: Asset, options: Dict[str, Any]) -> None:
        ...
        if metadata is not None:
            from app.api.utils.content_detection import detect_content_kind
            ...
            if new_kind is not None and new_kind != asset.kind:
                ...
                if get_content_type_registry().get_processor_class(asset) is None:
                    ...
                    self.session.add(asset)
                    self.session.commit()
                    self._run_phase3_enrichment(asset)
                    return
    ...
    def _run_phase3_enrichment(self, asset: Asset) -> None:
        """Run applicable enrichers on asset (language, quality score)."""
        from app.api.utils.enrichers import get_applicable_enrichers

        for enricher in get_applicable_enrichers(asset):
            try:
                enricher.run(asset, self.session)
            except Exception as e:
                logger.warning(
                    f"Enricher {enricher.name} failed on asset {asset.id}: {e}"
                )
        self.session.add(asset)
        self.session.commit()
