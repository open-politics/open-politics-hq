...
``details`` is that source's read-config. This module owns their CRUD +
...
def list_sources(
    ...
    sources = list(session.exec(query))
...
def update_source(
    ...
    """Update a source."""
    source = get_source(session, source_id, user_id, infospace_id)
    if not source:
        return None
...
def trigger_source_processing(
    ...
    try:
        ...
        job = run_source_ingestion(session, source_id)
...
def activate_stream(session: Session, source_id: int, user_id: int) -> Source:
    """
...
def pause_stream(session: Session, source_id: int, user_id: int) -> Source:
    """
...
def get_stream_stats(session: Session, source_id: int, user_id: int, infospace_id: int) -> Dict[str, Any]:
    """
