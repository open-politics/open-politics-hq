...
class FlowService:
    ...
    def _get_delta_assets(self, flow: Flow) -> List[int]:
        ...
        Uses high-water-mark cursor: asset.id > last_processed_asset_id for O(1) index scan.
        Falls back to processed_asset_ids list if cursor has legacy format.
        """
        ...
        if flow.input_type == FlowInputType.BUNDLE:
            if not flow.input_bundle_id:
                return []
            last_id = cursor.get("last_processed_asset_id")
            if last_id is not None:
                ...
                processed_ids = set(cursor.get("processed_asset_ids", []))
    ...
    def _update_cursor(self, flow: Flow, processed_asset_ids: List[int]) -> None:
        ...
        Uses high-water-mark: last_processed_asset_id = max(ids) for O(1) delta queries.
        Migrates from legacy processed_asset_ids list when first using high-water-mark.
        """
        if not processed_asset_ids:
            return
        cursor = flow.cursor_state or {}
        existing_max = cursor.get("last_processed_asset_id")
        new_max = max(processed_asset_ids)
        if existing_max is not None:
            new_max = max(existing_max, new_max)
        cursor["last_processed_asset_id"] = new_max
        ...
        if "processed_asset_ids" in cursor:
            del cursor["processed_asset_ids"]
