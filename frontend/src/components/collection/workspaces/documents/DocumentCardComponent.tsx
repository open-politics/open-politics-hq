...
const DocumentCardComponent: React.FC<DocumentCardComponentProps> = React.memo(({ 
  ...
  const { getIngestTaskForDataSource } = useRecurringTasksStore();

  return (
    <ScrollArea className="h-full max-w-[100vw]">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-2">
        {items.map((item) => {
          const ingestTask = item.type === 'url_list' ? getIngestTaskForDataSource(item.id) : null;
          const isScheduledIngestionActive = !!ingestTask && ingestTask.status === 'active';
                      ...
                      {isScheduledIngestionActive && (
                        <TooltipProvider delayDuration={100}>
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>Scheduled ingestion is active (Schedule: {ingestTask.schedule})</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      )}
