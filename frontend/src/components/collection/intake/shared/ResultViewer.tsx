...
import { resolveMarkdownUrls, formatPercentage } from '@/lib/format';
                ...
                <div className="grid grid-cols-2 gap-3 max-h-80 overflow-auto">
                  {result.source_metadata?.tavily_images?.map((image, idx) => (
                    <div key={idx} className="group relative">
                      ...
                      />
                      {image.description && (
                        <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs p-2 rounded-b-lg opacity-0 group-hover:opacity-100 [@media(pointer:coarse)]:opacity-100 transition-opacity">
                          {image.description}
                        </div>
                      )}
                    </div>
