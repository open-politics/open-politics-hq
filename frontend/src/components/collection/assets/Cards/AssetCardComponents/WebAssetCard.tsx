...
/**
 * WebAssetCard - Card for web/scraped content
 * 
 * Similar to ArticleAssetCard but with web-specific styling:
 * - Globe icon and sky-blue theme
 * - Source domain display
 * - External link indicator
 * - Text content preview
 * 
 * Shows text fallback immediately when no metadata image available.
 */
...
// Get image from file_info (synchronous)
function getMetadataImage(asset: AssetRead): FeaturedImageInfo | null {
  ...
  if (topImage && !isGifImage(topImage)) {
    return {
      url: topImage,
      alt: asset.title || 'Web content image',
...
function getChildImage(childAssets: AssetRead[]): FeaturedImageInfo | null {
  ...
  if (featuredChild) {
    ...
    if (imgUrl && !isGifImage(imgUrl)) {
      return {
        url: imgUrl,
        alt: featuredChild.title || 'Web content image',
  ...
  if (firstImage) {
    ...
    if (imgUrl) {
      return {
        url: imgUrl,
        alt: firstImage.title || 'Web content image',
...
interface WebAssetCardProps extends TypeSpecificCardProps {
  /** Custom fetch function for blob images */
...
export function WebAssetCard({
  ...
  const meta = { ...(asset.file_info ?? {}), ...(asset.facets ?? {}) } as Record<string, unknown>;
  const author = meta?.author;
  const publishedDate = meta?.publication_date || meta?.published_date;
  const sourceUrl = asset.source_identifier;
          ...
          {isCompact ? (
            <>
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground mt-auto">
                {domain && (
                  <span className="flex items-center gap-1 text-sky-600 dark:text-sky-400 truncate max-w-[80px]">
            ...
            <>
              ...
              {showMeta && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground mt-auto pt-1.5 border-t border-border/50">
                  ...
                  {author && (
                    <span className="truncate max-w-[100px]">By {author}</span>
                  )}
                  {fragmentCount > 0 && (
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <CardContent className={cn(
          ...
          {/* Title - only when we have an image */}
          {hasImage && (
