...
export default function RssFeedBrowser({
  ...
  const previewDiscoveredFeed = async (feedUrl: string, feedTitle: string) => {
    ...
    try {
      const data = await AssetsService.previewRssFeed({
        infospaceId: infospaceId!,
        feedUrl: feedUrl,
        maxItems: 20
      });
  ...
  const previewFeed = async () => {
    ...
    try {
      ...
      setPreviewData(data);
      setSelectedArticles(new Set());
      toast.success(`Previewed feed: ${(data as any).feed_info.title}`);
    } catch (err: any) {
      setError(err.message || "Failed to preview feed");
      console.error("Feed preview error:", err);
    } finally {
  ...
  const ingestSelectedArticles = async () => {
    ...
    try {
      ...
      const data = await AssetsService.ingestSelectedArticles({
        infospaceId: infospaceId!,
        feedUrl: previewFeedUrl,
        requestBody: selectedItems,
        bundleId: destination === 'existing_bundle' ? Number(selectedBundleId) : undefined
      });
      ...
      toast.success(`Successfully ingested ${(data as any).ingested_count} articles`);
      setSelectedArticles(new Set());
    } catch (err: any) {
  ...
  const createRssSource = async (autoMonitor: boolean = false) => {
    ...
    try {
      const data = await SourcesService.createRssSource({
        infospaceId: infospaceId!,
        requestBody: {
      ...
      toast.success(`Created RSS source: ${data.name}`);
    } catch (err: any) {
