...
if [ -n "$HETZNER_API_TOKEN" ]; then
    echo "   Scanning for all 'open-politics' resources..."
    ...
    LB_LIST=$(curl -s -H "Authorization: Bearer $HETZNER_API_TOKEN" \
        ...
        jq -r --arg slug "$CLUSTER_SLUG" --arg net "$NETWORK_ID" '
          .load_balancers[]? | select((.name | test("^open-politics-" + $slug + "-")) or (.private_net[]?.network == ($net|tonumber))) | .id' 2>/dev/null || echo "")
...
if [ -n "$HETZNER_API_TOKEN" ]; then
    ...
    REMAINING_RESOURCES=$(curl -s -H "Authorization: Bearer $HETZNER_API_TOKEN" \
        "https://api.hetzner.cloud/v1/servers" 2>/dev/null | \
        jq -r '.servers[]? | select(.name | test("open-politics")) | .name' 2>/dev/null | wc -l | tr -d ' ')
    
    if [ "$REMAINING_RESOURCES" -eq 0 ]; then
        echo "   ✅ All 'open-politics' resources successfully removed from Hetzner Cloud!"
    else
