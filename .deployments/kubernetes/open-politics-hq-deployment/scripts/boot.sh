...
EMAIL=$(grep '^email:' helm-chart/values.yaml | cut -d' ' -f2 | tr -d '"')
...
# 6. Deploy the Open Politics Application via Helm
print_header "Deploying Open Politics Application"
helm upgrade --install open-politics-stack ./helm-chart \
