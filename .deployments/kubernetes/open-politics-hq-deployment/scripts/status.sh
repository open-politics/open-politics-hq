...
# Namespaces
echo "📁 Namespaces:"
kubectl get namespaces | grep -E "(kube-system|open-politics)"
echo ""
...
# Application status
echo "🚀 Open Politics Application:"
kubectl get pods -n open-politics
echo ""

echo "📊 Application Services:"
kubectl get services -n open-politics
echo ""

echo "🔐 TLS Secrets (auto-generated by Traefik):"
kubectl get secrets -n open-politics | grep "tls" || echo "   TLS secrets will be auto-created by Traefik ACME"
echo ""

echo "💾 Persistent Volumes:"
kubectl get pv,pvc -n open-politics 2>/dev/null || echo "   No persistent volumes found"
echo ""

# Ingress status
echo "🌐 Ingress:"
kubectl get ingress -n open-politics
echo ""
...
echo "   Pods (open-politics namespace):"
kubectl top pods -n open-politics 2>/dev/null || echo "   ❌ Metrics not available (metrics-server not installed)"
echo ""

# Recent events
echo "📋 Recent Events (last 10):"
kubectl get events --sort-by=.metadata.creationTimestamp -n open-politics | tail -10
echo ""
...
if [ -n "$PROXY_IP" ]; then
    ...
    echo "   • Check certificate: kubectl get certificates -n open-politics"
    echo ""
fi
...
if [ -n "$PROXY_IP" ]; then
    ...
    echo "   📝 DNS Records to Create:"
    echo "   -------------------------"
    echo "   A     open-politics.org        → $PROXY_IP"
    echo "   A     www.open-politics.org    → $PROXY_IP"
    echo ""
    echo "   📋 Test when DNS propagates:"
    echo "   ----------------------------"
    echo "   curl -H \"Host: open-politics.org\" http://$PROXY_IP"
    echo "   curl -H \"Host: www.open-politics.org\" http://$PROXY_IP"
    echo ""
fi
...
if [ -n "$PROXY_IP" ]; then
    ...
    echo "   Production: https://open-politics.org (after DNS setup)"
fi
