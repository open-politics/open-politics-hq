...
LB_LIST=$(curl -s -H "Authorization: Bearer $HETZNER_API_TOKEN" \
    ...
    jq -r --arg slug "$CLUSTER_SLUG" --arg net "$NETWORK_ID" '
      .load_balancers[]
      | select((.name | test("^open-politics-" + $slug + "-")) or (.private_net[]?.network == ($net|tonumber)))
...
if [ -n "$LB_LIST" ]; then
    ...
    echo "   ✅ No Load Balancers found with 'open-politics' in name."
fi

echo

# Check for Networks with open-politics in the name
echo "🌐 Networks:"
NETWORK_LIST=$(curl -s -H "Authorization: Bearer $HETZNER_API_TOKEN" \
    "https://api.hetzner.cloud/v1/networks" | \
    jq -r '.networks[] | select(.name | contains("open-politics")) | "\(.id) \(.name)"' 2>/dev/null || echo "")
...
if [ -n "$NETWORK_LIST" ]; then
    ...
    echo "   ✅ No Networks found with 'open-politics' in name."
fi
...
FIREWALL_LIST=$(curl -s -H "Authorization: Bearer $HETZNER_API_TOKEN" \
    ...
    jq -r '.firewalls[] | select(.name | contains("open-politics")) | "\(.id) \(.name)"' 2>/dev/null || echo "")
...
if [ -n "$FIREWALL_LIST" ]; then
    ...
    echo "   ✅ No Firewalls found with 'open-politics' in name."
fi
