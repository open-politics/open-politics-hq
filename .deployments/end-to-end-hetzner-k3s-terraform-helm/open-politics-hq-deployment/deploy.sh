...
function get_system_state() {
    ...
    if kubectl get namespace hq >/dev/null 2>&1 && helm status hq-stack -n hq >/dev/null 2>&1; then
        APP_STATE="DEPLOYED"
...
function deploy_application() {
    ...
    # Check if infrastructure is deployed and kubectl is connected (unless skipped)
    if [ "${SKIP_CHECKS:-false}" != "true" ]; then
        ...
        # Best-effort connect without enforcing state
        ./scripts/connect.sh || true
    fi
    
    # Pre-flight check for values.yaml
    if [ ! -f "hq-cluster-chart/values.yaml" ]; then
        echo "❌ Error: hq-cluster-chart/values.yaml file not found."
        echo "   Please copy the .example file and fill out your secrets."
        exit 1
    fi
    if grep -q "changeThis" "hq-cluster-chart/values.yaml"; then
        echo "❌ Error: Default placeholder values found in hq-cluster-chart/values.yaml."
...
function reroll_traefik_acme() {
    print_header "Re-rolling Traefik and Retrying ACME"
    echo "   Current domain from values.yaml: $(grep "^  host:" hq-cluster-chart/values.yaml | awk '{print $4}')"
    echo "   Restarting Traefik (PVC-safe rollout)..."
    if [ "${ACME_FORCE_RESET:-false}" = "true" ]; then
        echo "   ACME_FORCE_RESET=true → clearing /data/acme.json in Traefik before restart"
        kubectl -n kube-system exec deploy/traefik -- sh -c ': > /data/acme.json' || true
    fi
    ...
    kubectl -n kube-system logs deploy/traefik --since=5m | grep -iE 'acme|let.?s encrypt|connection|timeout' || true
    echo "   Done. If issuance still fails, ensure DNS A/AAAA records point to your LB."
}
...
function main_menu() {
    ...
    if [ "${SKIP_CHECKS:-false}" != "true" ] && [ "$INFRA_STATE" = "PARTIAL" ]; then
        echo ""
    ...
    elif [ "${SKIP_CHECKS:-false}" != "true" ] && [ "$INFRA_STATE" = "DEPLOYED" ] && [ "$KUBECTL_STATE" = "NOT_CONNECTED" ]; then
        echo ""
    ...
    elif [ "${SKIP_CHECKS:-false}" != "true" ] && [ "$INFRA_STATE" = "DEPLOYED" ] && [ "$KUBECTL_STATE" = "WRONG_CLUSTER" ]; then
        echo ""
    ...
    echo "11. Re-roll Traefik and retry ACME (fix stuck certificates)"
    echo "12. Destroy All Infrastructure"
    echo "13. SSH to Master Node"
    echo "0.  Exit"
    ...
    case $choice in
        ...
        2) 
            if [ "${SKIP_CHECKS:-false}" != "true" ] && [ "$INFRA_STATE" != "DEPLOYED" ]; then
                echo "❌ Error: Infrastructure must be fully deployed before deploying application."
                echo "   Current state: $INFRA_STATE"
                return
            fi
        ...
        11) reroll_traefik_acme ;;
        12) destroy_infrastructure ;;
        13) ./scripts/ssh-master.sh ;;
        0) exit 0 ;;
...
case "$CMD" in
    ...
    *)
        ...
        echo "  --skip-checks   Skip system state checks and menu pause"
