...
ADMIN_KEY_NAME="hq-admin-${TIMESTAMP}"
WORKER_KEY_NAME="hq-worker-${TIMESTAMP}"
...
function delete_old_ssh_keys() {
    ...
    log "Cleaning up old SSH keys starting with 'hq'..."
    ...
    echo "$keys_response" | jq -r '.ssh_keys[] | select(.name | startswith("hq")) | "\(.id) \(.name)"' | while read -r key_id key_name; do
...
function update_terraform_ssh_key_name() {
    ...
    mkdir -p "$BACKUP_DIR"
    local ARCHIVE="$BACKUP_DIR/backups-$(date +%Y%m%d).tar"

    if [ -f ".tfvars" ]; then
        tar -rf "$ARCHIVE" .tfvars 2>/dev/null || tar -cf "$ARCHIVE" .tfvars
        sed -i.tmp "s/ssh_key_name = \".*\"/ssh_key_name = \"$new_admin_key_name\"/g" ".tfvars"
        rm -f ".tfvars.tmp"
    fi
    
    if [ -f "main.tf" ]; then
        tar -rf "$ARCHIVE" main.tf 2>/dev/null || tar -cf "$ARCHIVE" main.tf
...
function cleanup_old_backups() {
    ...
    # Remove centralized daily archives older than 14 days
    if [ -d ".backups" ]; then
        find .backups -name "backups-*.tar" -type f -mtime +14 2>/dev/null | while read -r file; do
            if [ -n "$file" ]; then
                files_found=true
    ...
    # Also remove any legacy scattered backups (older than 7 days)
    log "Looking for legacy backup files older than 7 days..."
