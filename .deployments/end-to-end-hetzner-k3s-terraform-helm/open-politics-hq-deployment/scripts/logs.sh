...
show_pods() {
    echo "📋 Available pods in hq namespace:"
...
KUBECTL_CMD="kubectl logs -n hq --tail=$LINES $SINCE $FOLLOW"
...
case $COMPONENT in
    ...
    all)
        ...
        if [ -n "$FOLLOW" ]; then
            ...
            kubectl logs -f -n hq --selector=app=backend,app=frontend,app=celery-worker --tail=10
        fi
        ;;
    *)
